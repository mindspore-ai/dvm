...
def test_allreduce_basic(comm, rank, size, op_type):
    """
    Verifies the correctness of AllReduce operations (sum and max) on fixed-shape
    """
    np.random.seed(1)
    ...
    inputs = []
    for i in range(size):
        inputs.append(np.random.normal(0, 1, [256, 1024]).astype(np.float16))
    expect = inputs[0].copy()
    for i in range(1, size):
        if op_type == "sum":
            expect += inputs[i]
        else:
            expect = np.maximum(expect, inputs[i])

    x1 = t.load(inputs[rank])
    x2 = t.allreduce(op_type, x1)
    x3 = t.abs(x2)
    t.store_expect(x3, np.abs(expect), 0.005)
    res = t.run_check(True)
    assert res
...
def test_allreduce_general(comm, rank, size, op_type, shape_size):
    """
    Tests the AllReduce operation on 1D tensors with various sizes
    """
    np.random.seed(1)
...
def test_allreduce_bf16(comm, rank, size, shape_size, op_type):
    """
    Validates AllReduce functionality with BFloat16 precision
    """
    np.random.seed(2)
