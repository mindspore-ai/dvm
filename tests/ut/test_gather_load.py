...
def _gather(x, index, axis):
    ...
    out_shape = tuple(x.shape[:axis]) + tuple(index.shape) + tuple(x.shape[axis + 1:])
    out = np.zeros(out_shape, dtype=x.dtype)
    prefix = int(np.prod(x.shape[:axis])) if axis > 0 else 1
    suffix = int(np.prod(x.shape[axis + 1:])) if axis + 1 < x.ndim else 1
    gather_dim = x.shape[axis]
    flat_out = out.reshape(prefix, index.size, suffix)
    flat_index = index.reshape(-1)
    flat_x = x.reshape(prefix, gather_dim, suffix)
    for outer in range(prefix):
        for i, idx in enumerate(flat_index):
            if idx < 0:
                idx += gather_dim
            if 0 <= idx < gather_dim:
                flat_out[outer, i] = flat_x[outer, idx]
    return out
...
def test_gather_load_add(src_shape, index_shape, axis, dtype):
    t = Tester()
    ...
    z = t.add(y, bias)
    t.store_expect(z, _gather(x_np, index_np, axis) + bias)
    assert t.run_check()
...
def test_gather_load_round_tile(src_shape, index_np, axis, out_shape, dtype, tiles):
    ...
    z = t.add(y, 0.5)
    t.store_expect(z, np.broadcast_to(_gather(x_np, index_np, axis), out_shape) + 0.5)
    for tile in tiles:
        t.tile(*tile)
    t.codegen()
