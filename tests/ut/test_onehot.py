...
def test_update_domain(pass_opt):
    ...
    y0 = t.add(t.load(ax), 0.1)
    y1 = t.sub(t.load(bx), y0)
    expect_y1 = bx - (ax + 0.1)
...
@arg_mark(plat_marks=['platform_ascend910b'], level_mark='level0', card_mark='onecard', essential_mark='essential')
def test_onehot_broker():
    t = Tester()
    depth, axis = 20, 1
    shape1, shape2 = [32, 128], [32, 20, 128]
    ax = np.random.randint(0, depth, shape1, np.int32)
    bx = np.full(shape1, 1, np.int32)
    cx = np.full(shape1, 1, np.int32)
    x1 = t.load(ax)
    x2 = t.add(x1, t.load(bx))
    x3 = t.sub(x2, t.load(cx))
    x4 = t.one_hot(x3, depth, axis, 2.0, 0.0, "float32")
    t.store_expect(x2, ax + 1)
    bx = np.random.normal(0.0, 0.02, shape2).astype(np.float32)
    x5 = t.mul(x4, t.load(bx))
    e_x4 = _gen_expect(ax, depth, axis, 2.0, 0.0)
    t.store_expect(x5, e_x4 * bx)
    x6 = t.add(x4, 0.1)
    t.store_expect(x6, e_x4 + 0.1)
    assert (t.run_check())
